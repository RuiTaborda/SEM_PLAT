...
class Wave:
    ...
    def n(self):
        if (2 * self.k() * self.h) > 10:
            # 5/math.sinh(10) = 0.0004539992985606108
            return 0.5
        else:
    ...
    def Q(self):
        if self.breaking:
            c = self.rho * math.sqrt(g) / (16*math.sqrt(self.breaker_index)*(self.rho_s - self.rho)*(1-self.p))
            return self.k * c * self.Hs**(5/2.) * math.sin(2*math.radians(self.alpha()))
